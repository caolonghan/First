...
- (void)setupJsContent
{
    ...
    context[@"wap_pay"] = ^(NSString* method) {
        NSLog(@"%@",method);
        [HttpClient sharedClient].requestSerializer = [AFJSONRequestSerializer serializer];
        [[HttpClient sharedClient].requestSerializer setValue:@"application/json" forHTTPHeaderField:@"Content-type"];
        ...
        [HttpClient sharedClient].responseSerializer.acceptableContentTypes = acctSet;
        
        [HttpClient requestWithMethod:@"POST" path:
         [NSString stringWithFormat:@"%@://%@/pingplusplus/CreateOrders",[Global sharedClient].HTTP_S,[Global sharedClient].Shopping_Mall]
                           parameters:
         [[NSDictionary alloc]initWithObjectsAndKeys:orderId,@"order_id",@"global",@"type",method,@"channel",@"ios",@"userAgent",nil ] target:self success:^(NSDictionary *dic){dispatch_async(dispatch_get_main_queue(), ^{
            NSLog(@"%@",dic);
